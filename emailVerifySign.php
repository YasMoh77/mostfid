...
if (!isset($session) ) { 
  ...
  if(isset($_SESSION['codeSignUpMos']) ){
      echo "<div class='text-center bottom'><span class='span-green'>".$lang['sucessRegister']."</span></div>";
    }  ?>
    

  <div class="top-emailVerify">
    <div class="centered"><?php echo $lang['sentCode']?> 
        <?php if(isset($_SESSION['codeSignUpMos'])){echo '( '.$_SESSION['codeSignUpMos'][0].' )';}
    ...
    <form  id="form-verify">
      ...
      <input type="hidden" name="email" value="<?php if(isset($_SESSION['codeSignUpMos'])){echo $_SESSION['codeSignUpMos'][0];}elseif(isset($_SESSION['codeUpdate'])){echo $_SESSION['codeUpdate'][0];} ?>">
    	<button type="submit" id="submit-verify" value="">تحقق  </button>
    </form>
